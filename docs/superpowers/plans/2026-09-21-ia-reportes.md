...
### Task 22: Los números, verificados contra Postgres real

**Se ejecuta después de la Task 13**, no al final: en cuanto existen las 18 métricas, antes
de construir la capa de LLM encima.

**Por qué existe esta tarea.** Las pruebas de las Tasks 4 y 8-12 simulan `dataSource.query`
y aciertan sobre el **texto** del SQL generado. Eso prueba que la consulta se arma como se
espera, pero **no prueba ni un solo número**. Durante la ejecución de este plan, el catálogo
pasó la suite entera en verde referenciando una columna que no existe (`created_at` en vez
de `"createdAt"`): el fallo solo aparecía contra Postgres real. Como toda la justificación
de elegir ficha-estructurada sobre texto-a-SQL fue *"los números no pueden salir mal"*, sin
esta tarea esa decisión quedaría sin respaldo.

El spec §8 ya lo pedía: *"una por cada una de las 18 métricas, con datos sembrados de
resultado conocido — la prueba compara contra un número calculado a mano"*.

**Files:**
- Create: `mirroria-backend/test/motor-consulta.e2e-spec.ts`

**Interfaces:**
- Consumes: `AppModule`, `MotorConsultaService.ejecutar(ficha, rango?)`, `FichaConsultaDto`.

- [ ] **Step 1: Escribir la siembra y la primera prueba que falla**

La siembra usa SQL directo con uuid fijos: es determinista y no depende de ningún servicio.
Todo ocurre dentro de un esquema de datos propio que se borra al final.

```ts
// test/motor-consulta.e2e-spec.ts
...
La siembra y la limpieza, en el mismo archivo:
...
async function sembrar(ds: DataSource): Promise<void> {
  ...
  await q(`INSERT INTO ordenes_compra (id, proveedor_id, sucursal_destino_id, usuario_id,
           ...
           VALUES ($1, $2, $3, $4, 'RECIBIDA_PARCIAL',
                   '[{"varianteId":"${ID.var1}","cantidadPedida":10,"cantidadRecibida":6}]'::jsonb,
                   '2026-08-05')`,
    [ID.orden1, ID.prov, ID.suc1, ID.cliente1]);
}
```

Y la primera prueba, la que fija el filtro de estado:
...
Run: `cd .. && docker compose up -d postgres-db && cd mirroria-backend && npm run test:e2e`
Expected: el archivo nuevo falla (o la siembra rompe) porque todavía no está escrito completo.

- [ ] **Step 3: Completar las pruebas, una por dominio**
...
salvo que puedas demostrar con `psql` que el número esperado estaba mal calculado.

- [ ] **Step 5: Commit**

```bash
git add test/motor-consulta.e2e-spec.ts
git commit -m "test(ia): los numeros del motor verificados contra Postgres real"
```
